...
import BlogForm from "@/Components/Segment/Backend/SampleScript/Blog/BlogFrom.jsx";
...
export default function Page({data: blogListData}){
    ...
    useEffect(() => {
        if (pagination?.page && pagination?.length && go) {
            ...
            router.get(route('admin.sample-script.blog.list', queryParams))
    ...
    const alert = (id) => {
        setAlertData({
            ...alertData,
            route : `/admin/sample-script/blog/delete/${id}`,
            color : 'red',
            method : 'delete',
                                            ...
                                            <td className="px-4 py-2 font-medium text-gray-800 dark:text-neutral-200 border border-neutral-300 dark:border-neutral-400">
                                                {
                                                    blog.status === STATUS_DELETED ? (<span className={`capitalize`}>{blog.status}</span>) : (
                                                        <select
                                                            ...
                                                            value={blog.status}
                                                            onChange={(e) => {
                                                                router.post(route('admin.sample-script.blog.change_status'), {
