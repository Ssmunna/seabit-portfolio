...
export default function Page({data:content}){
    const {bio_section} = content
    const {fileBase} = usePage().props

    const {data, setData, post, processing, errors, reset} = useForm({
        description: '',
        image: '',
        page_name: 'about-me-bio'
    });
    ...
    useEffect(() => {
        if(bio_section){
            setPreviewImage(`${fileBase}/${bio_section.image}`)
            setData({
                ...data,
                description: bio_section.description ?? '',
                id: bio_section.id ?? '',
            })
        } else {
            setData({
    ...
    const handleSubmitForm = (e) => {
        e.preventDefault();
        post(route('admin.about-me.bio.update'), {
            preserveState: true,
            preserveScroll: true,
        })
    }
                    ...
                    <form onSubmit={handleSubmitForm} className={`w-full space-y-2`}>
                        ...
                        <div className="form-control pt-4">
                            <div className="w-full flex gap-x-4">
                                <Button buttonText={!bio_section ? 'Create' : 'Update'} isLoading={processing} />
                            </div>
                        </div>
                    </form>
                </div>
